...
# plt.ylim(-0.01, 4)
# plt.tight_layout()
# fig.savefig('examples/penannular_proof/areas-2.png', dpi=300)
# plt.show()
...
def make_animation_of_scaling_sweep(tube_radius = 0.01,
                                    frames_folder='examples/linear_proof/scale-sweep-frames/',
                                    sphere_lines_are_thinner_by=4):
    core_radius = 1

    best_scale = 2*np.pi*(3/4)
    input_path = make_path(xlen=3.81, r=1.23, Npath=150) * best_scale
    input_path_single_section = make_path(xlen=3.81, r=1.23, Npath=150, do_double=False) * best_scale
...
make_animation_of_scaling_sweep(tube_radius = 0.05, frames_folder='examples/linear_proof/scale-sweep-frames/',
