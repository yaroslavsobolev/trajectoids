...
input_path_single_section = make_random_path(seed=folder_suffix, make_ends_horizontal=False, start_from_zero=True, end_with_zero=True, amplitude=3.5,
...
# best_scale = 0.6515761279968767
best_scale = 0.6380211241164975
...
for frame_id, scale_to_plot in enumerate(tqdm(scale_list)):
    ...
    # mlab.show()
    mlab.savefig(f'misc/figures/scale-sweep-two-periods-{folder_suffix}/frames-fixed-sphere/frame{frame_id:04d}.png', magnification=2)
    mlab.close(all=True)
x = input_path_0[:, 0]
y = input_path_0[:, 1]
...
circle_center=[5, 1]
circlealpha=1
path_for_figs = f'misc/figures/scale-sweep-two-periods-{folder_suffix}/frames-flat/'
for frame_id, scale_to_plot in enumerate(tqdm(scale_list)):
