...
def blend(blend_factor):
    return input_path_0 * (1-blend_factor) + input_path * blend_factor

nframes = 100
factor_list = np.linspace(0, 1, nframes)
...
for frame_id, factor_to_plot in enumerate(tqdm(factor_list)):
    figtraj = plt.figure(10, figsize=(10, 5))
    dataxlen = np.max(input_path[:, 0])
    savetofile=f'misc/figures/deformation-illustration/frames-flat/frame{frame_id:04d}'
    linewidth=3
    color = 'C0'
    ...
    # plot_periods(data, '--', linewidth=0.5)
    plot_periods(blend(factor_to_plot), '-', linewidth=linewidth)
    # plot_periods(projection_centers, '-', linewidth=1)
    ...
    for node_id in node_ids:
        startpoint = input_path_0[node_id, :]
        endpoint = blend(factor_to_plot)[node_id, :]
    ...
    for shift in dataxlen * np.arange(3):
        plt.scatter(shift + blend(factor_to_plot)[-1, 0], blend(factor_to_plot)[-1, 1], s=35, color='black')
...
def plot_spherical_one_period(input_path, scale, plotting_upsample_factor=1,
...
scale_to_plot = 1.28319
for frame_id, factor_to_plot in enumerate(tqdm(factor_list)):
    input_path_single_section = blend(factor_to_plot)
    ...
    mlab.savefig(f'misc/figures/deformation-illustration/frames-trace/frame{frame_id:04d}.png', magnification=2)
    mlab.close(all=True)
